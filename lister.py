...
class Listlnstance:
    ...
    instances via inheritance of
    coded here; displays
    str
    instance attrs only; self is the instance of lowest class;
    X names to avoid clashing with client's attrs
    uses
    """
    def __str__(self):
...
class ListTree:
    """
    Mix-in that returns an
    st r trace Of t he entire class
    tree and all its objects' attrs at and above self;
    run by print(), str() returns constructed string;
    uses _X attr names to avoid impacting clients;
